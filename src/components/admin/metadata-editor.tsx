        ...
        {metadata.map((item, index) => (
          <div key={item.id} className="flex items-end gap-2">
            <div className="flex-1 space-y-2">
              <Label>键名</Label>
              {item[isCustomKey] ? (
                <Input value={item.key} onChange={(e) => updateMetadata(index, 'key', e.target.value, false)} placeholder="自定义键名" />
              ) : (
                <Select value={item.key} onValueChange={(value) => updateMetadata(index, 'key', value, true)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="#custom" className="text-purple-500 focus:text-purple-500">
                      自定义
                    </SelectItem>
                  </SelectContent>
                </Select>
            ...
            <div className="flex-1 space-y-2">
              <Label>值</Label>
              <Input value={item.value} onChange={(e) => updateMetadata(index, 'value', e.target.value, false)} placeholder="元数据值" />
            </div>
